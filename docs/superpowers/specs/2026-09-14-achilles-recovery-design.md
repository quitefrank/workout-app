...
| `authoring.ts` | `checkExercise(exercise, state, equipment, position, previous)`, `checkTemplate(template, state, equipment)`, `checkSpacing(templates)` | Section 6. `position` is the 1-based slot in the template; `previous` is the row before it, which rule 8 reads (core work stays in one block at the end). `AuthoringExercise` carries `muscleGroup` for it |
...
| 8 | the previous row is core work (muscle group Abs) and this row is not; a core block at the end of a template passes | warn. Order follows training logic: biggest compound first, isolation after, core last. Until 2026-09-16 this rule put floor work first to save a transfer; Frank overruled it because the bench on and off is the move with the injury potential, and floor transfers no longer order a template |
